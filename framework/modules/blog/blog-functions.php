		...
		if (1 != $pages) {
			...
			for ($i = 1; $i <= $pages; $i++) {
				if (1 != $pages && (!($i >= $paged + $range + 1 || $i <= $paged - $range - 1) || $pages <= $showitems)) {
					echo readanddigest_elated_get_module_part(($paged == $i) ? "<li class='active'><span>" . $i . "</span></li>" : "<li><a href='" . get_pagenum_link($i) . "' class='inactive'>" . $i . "</a></li>");
				}
			}
				...
				<div class="eltdf-comment-text-and-info">
					<div class="eltdf-comment-info-and-links">
						<h6 class="eltdf-comment-name">
							<?php if ($is_pingback_comment) {
								esc_html_e('Pingback:', 'readanddigest');
							} ?><span class="eltdf-comment-author"><?php echo wp_kses_post(get_comment_author_link()); ?></span>
		...
		if (is_singular('post')) {
			if (isset($_COOKIE['eltd-post-views_' . $postID])) {
				...
				if ($count === '') {
					...
					setcookie('eltd-post-views_' . $postID, $postID, intval(time() * 20), '/');
				} else {
					$count++;
	...
	$term_meta = get_option("post_tax_term_$t_id");
			...
			<td>
				<select name="term_meta[template]" id="term_meta[template]">
					<option <?php if (isset($term_meta['template']) && $term_meta['template'] == 'type1') {
							...
							} ?> value='type1'><?php esc_html_e('Template 1', 'readanddigest'); ?></option>
					<option <?php if (isset($term_meta['template']) && $term_meta['template'] == 'type2') {
								echo "selected='selected'";
							} ?> value='type2'><?php esc_html_e('Template 2', 'readanddigest'); ?></option>
					<option <?php if (isset($term_meta['template']) && $term_meta['template'] == 'type3') {
								echo "selected='selected'";
							} ?> value='type3'><?php esc_html_e('Template 3', 'readanddigest'); ?></option>
					<option <?php if (isset($term_meta['template']) && $term_meta['template'] == 'type4') {
								echo "selected='selected'";
							} ?> value='type4'><?php esc_html_e('Template 4', 'readanddigest'); ?></option>
					<option <?php if (isset($term_meta['template']) && $term_meta['template'] == 'type5') {
								echo "selected='selected'";
							} ?> value='type5'><?php esc_html_e('Template 5', 'readanddigest'); ?></option>
					<option <?php if (isset($term_meta['template']) && $term_meta['template'] == 'type6') {
								echo "selected='selected'";
							} ?> value='type6'><?php esc_html_e('Template 6', 'readanddigest'); ?></option>
				</select>
			</td>
		</tr>
		<tr class="form-field">
			...
			<td>
				<select name="term_meta[sidebar_layout]" id="term_meta[sidebar_layout]">
					...
					<option <?php if (isset($term_meta['sidebar_layout']) && $term_meta['sidebar_layout'] == 'default') {
								echo "selected='selected'";
							} ?> value='default'><?php esc_html_e('No Sidebar', 'readanddigest'); ?></option>
					<option <?php if (isset($term_meta['sidebar_layout']) && $term_meta['sidebar_layout'] == 'sidebar-33-right') {
								echo "selected='selected'";
							} ?> value='sidebar-33-right'><?php esc_html_e('Sidebar 1/3 Right', 'readanddigest'); ?></option>
					<option <?php if (isset($term_meta['sidebar_layout']) && $term_meta['sidebar_layout'] == 'sidebar-25-right') {
								echo "selected='selected'";
							} ?> value='sidebar-25-right'><?php esc_html_e('Sidebar 1/4 Right', 'readanddigest'); ?></option>
					<option <?php if (isset($term_meta['sidebar_layout']) && $term_meta['sidebar_layout'] == 'sidebar-33-left') {
								echo "selected='selected'";
							} ?> value='sidebar-33-left'><?php esc_html_e('Sidebar 1/3 Left', 'readanddigest'); ?></option>
					<option <?php if (isset($term_meta['sidebar_layout']) && $term_meta['sidebar_layout'] == 'sidebar-25-left') {
								echo "selected='selected'";
							} ?> value='sidebar-25-left'><?php esc_html_e('Sidebar 1/4 Left', 'readanddigest'); ?></option>
				</select>
			</td>
		</tr>
		<tr class="form-field">
				...
				<select name="term_meta[custom_sidebar]" id="term_meta[custom_sidebar]">
					...
					foreach ($custom_sidebars as $key => $value) { ?>
						<option <?php if (isset($term_meta['custom_sidebar']) && $term_meta['custom_sidebar'] == $key) {
									echo "selected='selected'";
								} ?> value='<?php echo readanddigest_elated_get_module_part($key); ?>'><?php echo readanddigest_elated_get_module_part($value); ?></option>
					<?php } ?>
		...
		<tr>
			...
			<td>
				<div style="display: inline;">
					...
					<input style="width:100px" type="text" name="term_meta[featured_thumb_image_width]" id="term_meta[featured_thumb_image_width]" size="3" value="<?php if (isset($term_meta['featured_thumb_image_width']) && $term_meta['featured_thumb_image_width'] != '') {
																																										echo esc_attr($term_meta['featured_thumb_image_width']);
																																									} ?>">
				</div>
				<div style="display: inline;">
					<label style="margin-right:5px" for="shortcode"><?php esc_html_e('Custom Image Height(px)', 'readanddigest'); ?></label>
					<input style="width:100px" type="text" name="term_meta[featured_thumb_image_height]" id="term_meta[featured_thumb_image_height]" size="3" value="<?php if (isset($term_meta['featured_thumb_image_height']) && $term_meta['featured_thumb_image_height'] != '') {
																																											echo esc_attr($term_meta['featured_thumb_image_height']);
																																										} ?>">
				</div>
				<div style="display: inline;">
					<label style="margin-right:5px" for="shortcode"><?php esc_html_e('Excerpt Length', 'readanddigest'); ?></label>
					<input style="width:100px" type="text" name="term_meta[featured_excerpt_length]" id="term_meta[featured_excerpt_length]" size="3" value="<?php if (isset($term_meta['featured_excerpt_length']) && $term_meta['featured_excerpt_length'] != '') {
																																									echo esc_attr($term_meta['featured_excerpt_length']);
																																								} ?>">
				</div>
			</td>
		</tr>
		<tr>
			...
			<td>
				<div style="display: inline;">
					<label style="margin-right:5px" for="shortcode"><?php esc_html_e('Custom Image Width(px)', 'readanddigest'); ?></label>
					<input style="width:100px" type="text" name="term_meta[featured_small_thumb_image_width]" id="term_meta[featured_small_thumb_image_width]" size="3" value="<?php if (isset($term_meta['featured_small_thumb_image_width']) && $term_meta['featured_small_thumb_image_width'] != '') {
																																													echo esc_attr($term_meta['featured_small_thumb_image_width']);
																																												} ?>">
				</div>
				<div style="display: inline;">
					<label style="margin-right:5px" for="shortcode"><?php esc_html_e('Custom Image Height(px)', 'readanddigest'); ?></label>
					<input style="width:100px" type="text" name="term_meta[featured_small_thumb_image_height]" id="term_meta[featured_small_thumb_image_height]" size="3" value="<?php if (isset($term_meta['featured_small_thumb_image_height']) && $term_meta['featured_small_thumb_image_height'] != '') {
																																														echo esc_attr($term_meta['featured_small_thumb_image_height']);
																																													} ?>">
				</div>
				<div style="display: inline;">
					<label style="margin-right:5px" for="shortcode"><?php esc_html_e('Excerpt Length', 'readanddigest'); ?></label>
					<input style="width:100px" type="text" name="term_meta[featured_small_excerpt_length]" id="term_meta[featured_small_excerpt_length]" size="3" value="<?php if (isset($term_meta['featured_small_excerpt_length']) && $term_meta['featured_small_excerpt_length'] != '') {
																																												echo esc_attr($term_meta['featured_small_excerpt_length']);
																																											} ?>">
				</div>
			</td>
		</tr>
			...
			<td>
				<select name="term_meta[pagination_type]" id="term_meta[pagination_type]">
					<option <?php if (isset($term_meta['pagination_type']) && $term_meta['pagination_type'] == 'np-horizontal') {
								echo "selected='selected'";
							} ?> value='np-horizontal'><?php esc_html_e('Horizontal Navigation', 'readanddigest'); ?></option>
					<option <?php if (isset($term_meta['pagination_type']) && $term_meta['pagination_type'] == 'load-more') {
								echo "selected='selected'";
							} ?> value='load-more'><?php esc_html_e('Load More', 'readanddigest'); ?></option>
					<option <?php if (isset($term_meta['pagination_type']) && $term_meta['pagination_type'] == 'infinite') {
								echo "selected='selected'";
							} ?> value='infinite'><?php esc_html_e('Infinite Scroll', 'readanddigest'); ?></option>
				</select>
			</td>
		</tr>
	<?php
